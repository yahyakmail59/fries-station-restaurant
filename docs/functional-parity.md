...
| C16 | تبديل الاستلام/التوصيل | `input[name="fulfillment"]` | `RestaurantSettings.delivery_enabled`, `base.html`, `main.js` | الاستلام ظاهر دائمًا؛ التوصيل يظهر فقط عند تشغيله من إعدادات المطعم ثم يُظهر `#delivery-fields` | مفتاح تشغيل آمن من اللوحة | الحقول تظهر وتختفي عند التفعيل | معطّل ← لا خيار توصيل | عام | `test_delivery_can_be_enabled_from_restaurant_settings` | ✅ |
...
| C18 | تحقق التوصيل (خادم) | `POST /order/` | `views_order.create_order` | يرفض التوصيل المعطّل أولًا، ثم يتحقق من الاسم والهاتف والعنوان عند تشغيله | **لا يجوز الاعتماد على إخفاء الواجهة وحده** | طلب صحيح عند التفعيل | معطّل أو بيانات ناقصة ← 400 | عام | `test_disabled_delivery_is_refused_even_with_complete_contact_details` · `test_delivery_requires_name_phone_and_address` | ✅ |
